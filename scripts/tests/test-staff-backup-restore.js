...
async function run() {
  ...
  const custRes = await makeRequest('POST', '/api/customers', staffHeaders, {
    ...
    address: 'Komarapalayam Test St'
  });
  assert('Created test customer', custRes.status === 200 || custRes.status === 201);
  const testCustId = custRes.data?.data?.id || custRes.data?.id || 'CUST-0022';
  ...
  assert('Validation status is VALID', valRes.data?.data?.status === 'VALID');
  assert('All SHA-256 file checksums verified', valRes.data?.data?.manifest?.recordCounts?.customers >= 1);
  ...
  const foundTestCust = restoredCustomers.find(c => c.id === testCustId || c.name === 'Backup Test Customer');
